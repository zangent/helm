...
Type *type_to_abi_compat_param_type(gbAllocator a, Type *original_type) {
	...
	if (build_context.HELM_ARCH == "x86") {
		return new_type;
	}

	if (build_context.HELM_OS == "windows") {
		...
		// Especially the only Helm types
		case Type_Basic: {
...
bool check_procedure_type(Checker *c, Type *type, AstNode *proc_type_node, Array<Operand> *operands) {
	...
	if (param_count > 0) {
		...
		if (end->flags&EntityFlag_CVarArg) {
			if (pt->calling_convention == ProcCC_Helm) {
				error(end->token, "Helm calling convention does not support #c_vararg");
			} else if (pt->calling_convention == ProcCC_Contextless) {
				error(end->token, "Helm's contextless calling convention does not support #c_vararg");
...
bool check_builtin_procedure(Checker *c, Operand *operand, AstNode *call, i32 id) {
	...
	case BuiltinProc_append: {
		...
		Type *proc_type = make_type_proc(c->allocator, nullptr, proc_type_params, 2, nullptr, false, true, ProcCC_Helm);
