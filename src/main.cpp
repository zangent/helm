...
void usage(String argv0) {
	print_usage_line(0, "%.*s is a tool for managing Helm source code", LIT(argv0));
	print_usage_line(0, "Usage:");
	print_usage_line(1, "%.*s command [arguments]", LIT(argv0));
	print_usage_line(0, "Commands:");
	print_usage_line(1, "build        compile .helm file as executable");
	print_usage_line(1, "build_dll    compile .helm file as dll");
	print_usage_line(1, "run          compile and run .helm file");
	print_usage_line(1, "docs         generate documentation for a .helm file");
...
int main(int arg_count, char **arg_ptr) {
	...
	if (!find_library_collection_path(str_lit("shared"), nullptr)) {
		add_library_collection(str_lit("shared"),
		                       get_fullpath_relative(heap_allocator(), helm_root_dir(), str_lit("shared")));
	}
	...
	{ // CTFE
		/*
		BuildContext backup_build_context = build_context;
		...
		llvm_build(&timings, &checker);

		build_context = backup_build_context;
		// */
	}
...
#else
	int llvm_ret = llvm_build(&timings, &checker);
	...
	i32 exit_code = 0;
		...
		exit_code = system_exec_command_line_app("llvm-opt", false,
			...
			"-die "
				...
				// NOTE: If you change this (although this minimum is as low as you can go with Helm working)
